...
export const PlayerDashboard = ({ handleMoveClick }: PlayerDashboardProps) => {
  ...
  const { getEffectivenessColor } = useMoveAnalysis();

  // Optimization: Memoize stat values
        ...
        {playerTurn ? (
          <div className="grid grid-cols-2 gap-2 h-full">
            {player.moves?.map((move, index) => {
              ...
              const effectivenessClass = getEffectivenessColor(move);
              return (
                <button
                  key={index}
                  onClick={() => handleMoveClick(move)}
                  disabled={isOutOfPP}
                  className={`relative p-2 rounded border-2 text-left transition-all flex flex-col justify-between group cursor-pointer ${effectivenessClass} ${
                    isOutOfPP 
                      ? 'bg-gray-900 border-gray-900 opacity-40 cursor-not-allowed' 
                      : 'bg-gray-800 border-white hover:bg-white'
                  }`}
                >
                  ...
                  <div className="flex justify-between items-end w-full mt-1">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`text-[9px] md:text-[10px] font-black transition-colors ${
                        isOutOfPP ? 'text-red-900' : move.pp < 3 ? 'text-red-500' : 'text-white group-hover:text-black'
                      }`}>
      ...
      <div className='order-2 md:order-2 bg-slate-900 border-b-4 md:border-b-0 md:border-t-4 border-black flex flex-col relative overflow-hidden font-mono text-white mt-0 md:mt-6'>
        ...
        <div className='p-2 flex justify-between items-center border-b-2 md:border-b-4 border-black bg-slate-800'>
           ...
           {player.equipment && player.equipment.length > 0 && (
             <span className="text-[8px] md:text-[10px] text-green-400 font-bold animate-pulse">LINK ({player.equipment.length}/6)</span>
           )}
        </div>
