...
export const StartScreen = ({ highScore, startGame }: StartScreenProps) => {
  ...
  useEffect(() => {
    ...
    if (savedName && !userProfile) {
      setUserProfile({ name: savedName, email: '', picture: '' });
    }
  ...
  const handleSetName = () => {
    ...
    if (!trimmed) return;
    setUserProfile({ name: trimmed, email: '', picture: '' });
          ...
          {!userProfile ? (
            <>
              <p className='text-[10px] md:text-xs text-gray-400 mb-3 text-center uppercase tracking-widest'>
                Enter a name to record your runs
              </p>
            ...
            <div className="text-center">
              <p className="text-xs md:text-sm text-green-400 font-bold uppercase tracking-widest mb-2">
