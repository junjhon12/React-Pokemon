...
export const getEffectiveStat = (mon: Pokemon, stat: StatKey, modifier: DungeonModifier = 'none') => {
  ...
  const stageStats: StageStatKey[] = ['attack', 'defense', 'specialAttack', 'specialDefense', 'speed'];
  if (mon.stages && stageStats.includes(stat as StageStatKey)) {
    finalValue = applyStatStage(finalValue, mon.stages[stat as StageStatKey] ?? 0);
  }
