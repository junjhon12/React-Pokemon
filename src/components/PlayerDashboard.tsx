...
export const PlayerDashboard = ({ handleMoveClick }: PlayerDashboardProps) => {
  ...
  const { getMoveEffectivenessClass } = useMoveAnalysis();
  ...
  const memoizedStats = useMemo(() => {
    ...
    return {
      ...
      specialAttack:  getEffectiveStat(player, 'specialAttack'),
      specialDefense: getEffectiveStat(player, 'specialDefense'),
  ...
  // ⚔ = physical, ✦ = special
  const getDamageClassIcon = (move: Move) => {
    if (move.damageClass === 'special') return '✦';
        ...
        {playerTurn ? (
          <div className="grid grid-cols-2 gap-2 h-full">
            {player.moves?.map((move, index) => {
              ...
              const matchupClass = getMoveEffectivenessClass(move);
              ...
              return (
                <button
                  ...
                  disabled={isOutOfPP}
                  className={`relative p-2 rounded border-2 text-left transition-all flex flex-col justify-between group cursor-pointer ${matchupClass} ${
                    isOutOfPP ? 'bg-gray-900 border-gray-900 opacity-40 cursor-not-allowed' : 'hover:bg-white/10'
                  }`}
                >
                  <div className="flex justify-between items-start w-full">
                    <span className={`font-bold text-[10px] md:text-xs uppercase truncate w-20 transition-colors ${!isOutOfPP ? 'text-white' : 'text-gray-500'}`}>
                      {move.name}
                    </span>
                    <div className="flex items-center gap-1">
                      {/* Damage class icon */}
                      <span className={`text-[9px] font-black ${move.damageClass === 'special' ? 'text-blue-300' : move.damageClass === 'physical' ? 'text-orange-300' : 'text-gray-500'}`}>
                        {getDamageClassIcon(move)}
                      </span>
                      <div className={`w-2 h-2 rounded-full ${getTypeColor(move.type)} shadow-sm`} />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-end w-full mt-1">
                    <div className="flex gap-2 text-[8px] md:text-[9px]">
                      <div className="flex flex-col">
                        ...
                        <span className={`font-bold ${!isOutOfPP ? 'text-gray-200' : 'text-gray-600'}`}>{move.power || '--'}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-gray-500 uppercase leading-none">Acc</span>
                        <span className={`font-bold ${!isOutOfPP ? 'text-gray-200' : 'text-gray-600'}`}>{move.accuracy}%</span>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <span className={`text-[9px] md:text-[10px] font-black ${
                        isOutOfPP ? 'text-red-900' : move.pp < 3 ? 'text-red-500' : 'text-white'
                      }`}>
          ...
          <div className="h-full flex items-center justify-center">
            <p className='text-red-500 font-bold text-lg md:text-xl animate-pulse uppercase'>Wait...</p>
          </div>
        )}
      </div>
      ...
      <div className='order-3 md:order-1 p-4 md:p-6 flex-1'>
        <div className='space-y-1 md:space-y-2 text-xs md:text-sm font-bold md:border-b-4 md:border-black md:pb-4'>
          <div className='flex justify-between'>
            <span className='text-red-700'>❤️ HP</span>
            <span>{player.stats.hp}/{player.stats.maxHp}</span>
          </div>
          <div className='flex justify-between'>
            <span className='text-orange-700'>⚔ Attack</span>
            <span>{memoizedStats?.attack}</span>
          </div>
          <div className='flex justify-between'>
            <span className='text-blue-600'>✦ Sp. Atk</span>
            <span>{memoizedStats?.specialAttack}</span>
          </div>
          <div className='flex justify-between'>
            <span className='text-blue-700'>🛡️ Defense</span>
            <span>{memoizedStats?.defense}</span>
          </div>
          <div className='flex justify-between'>
            <span className='text-indigo-600'>🔮 Sp. Def</span>
            <span>{memoizedStats?.specialDefense}</span>
          </div>
          <div className='flex justify-between'>
            <span className='text-yellow-600'>⚡ Speed</span>
            <span>{memoizedStats?.speed}</span>
          </div>
          <div className='flex justify-between text-gray-600 pt-2 border-t border-gray-400'>
            <span>🎯 Crit: {memoizedStats?.crit}%</span>
            <span>💨 Dodge: {memoizedStats?.dodge}%</span>
          </div>
        </div>
      </div>
    </div>
